var adaptation_state_actions = function adaptation_state_actions(graph, selected_method) {
  ...
  function serializeHardwareRelationshipsAction_Result(graph, dicElements) {
    ...
    let relations = getRelationsFromTypes(graph, "adaptation_behavior_hardware", ["readAction"], ["digitalVariable", "analogVariable", "stringVariable"]);
    for (var i = 0; i < relations.length; i++) {
      var relation = relations[i];
      var source = relation.source;
      var target = relation.target;
  ...
  function serializeHardwareRelationshipsAction_Variable(graph, dicElements) {
    ...
    let relations = getRelationsFromTypes(graph, "adaptation_behavior_hardware", ["digitalVariable", "analogVariable", "stringVariable"], ["actionArgument"]);
    for (var i = 0; i < relations.length; i++) {
      var relation = relations[i];
      var source = relation.source;
      var target = relation.target;
  ...
  function serializeBindingRelationshipsPredicate_Variables(graph, dicElements) {
    ...
    for (var i = 0; i < edges.length; i++) {
      ...
      var sourceType = source.getAttribute("type");
      var targetType = target.getAttribute("type");
      var relName = target.getAttribute("label") + "_variables";

      if ((sourceType == "digitalVariable" || sourceType == "analogVariable" || sourceType == "stringVariable") && targetType == "predicate") {
        ...
        if (!item) {
          item = {
            ...
            primaryVariable: dicElements.getId("binding", "variable", source.getAttribute("label")),
