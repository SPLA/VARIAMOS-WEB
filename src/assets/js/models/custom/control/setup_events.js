...
let setupEvents = function setupEvents(graph) {
  const texts = [
    "Input change or setpoint applied :  ",
    "Delay[𝜃] :    ",
    "Discrete Time :   ",
  ];
  const default_vals = ["", "", ""];
  ...
  const DataContinuous = () => {
    ...
    let footer = [];
    footer.push(
      modalButton("Import Data", function() {
        UploadData();
      })
    );
    footer.push(
      modalButton("Estimate", function() {
  ...
  const setupModalInformation = () => {
    ...
    const default_vals = [
      `Corresponds to "control input" or "manipulated variable".This is a quantity
  ...
  const SistemIdentification = () => {
    ...
    function taoValue() {
      ...
      for (let i = 1; i < dataList.length; i++) {
        if (dataList[i] == indexOfClosests(ConstantsFirstOrder().taoData)) {
          ...
          dataList[i] == indexOfClosests(ConstantsFirstOrder().TaoData28)
        ) {
    ...
    function nolinearProcess() {
      ...
      let funcionFirOrder =
        ...
        deltaValue +
        ") * (1-Exp( - (t-" +
        ConstantsFirstOrder().delayValue +
        " ) / a )))+ D ";
      function EXP(x) {
        return Math.exp(x);
      }
      ...
      for (i = 1; i <= nPts; i++) {
        ...
        SEest = w * SQRT(SEest);
        let yco = yc;
        let ycl = yc - St95 * SEest;
        let ych = yc + St95 * SEest;
    ...
    function Transformz() {
      let parameter = {
        qo:
          parametersZiegler().Kp *
          (1 + 1 / (2 * parametersZiegler().Ti) + parametersZiegler().Td / 1),
        q1:
          -parametersZiegler().Kp *
          (1 -
            1 / (2 * parametersZiegler().Ti) +
            (parametersZiegler().Td * 2) / 1),
    ...
    function canvasGraph(titleHeader, labels, datacsv, output) {
      ...
      let body = "";
      let footer = modalButton("return", function() {
        DataContinuous();
      });
      setupModal(header, body, footer);
      let transferFuncion;
      ...
      if (idDiscrete == true) {
        transferFuncion =
          "D(z) = \\frac{" +
          Transformz().qo.toFixed(2) +
          " z^2" +
          " " +
          Transformz().q1.toFixed(2) +
          " z" +
          " + " +
          Transformz().q2.toFixed(2) +
          "}{" +
          "z" +
          "(z-1)" +
          "}";
      ...
      var DataDisplay = [
        transferFuncion,
        "Ziegler–Nichols:  K:" +
          parametersZiegler().Kp.toFixed(2) +
          "\\" +
          "   " +
          " Ti:" +
          parametersZiegler().Ti.toFixed(2) +
          " " +
          " Td:" +
          parametersZiegler().Td.toFixed(2) +
          " ",
        "Cohen-Coon:  K:" +
          parametersCohen().Kp.toFixed(2) +
          "\\" +
          "   " +
          " Ti:" +
          parametersCohen().Ti.toFixed(2) +
          " " +
          " Td:" +
          parametersCohen().Td.toFixed(2) +
          " ",
        "Amigo:  K:" +
          parametersAmigo().Kp.toFixed(2) +
          "   " +
          " Ti:" +
          parametersAmigo().Ti.toFixed(2) +
          " " +
          " Td:" +
          parametersAmigo().Td.toFixed(2),
      ];
  ...
  function controlllerOutput() {
    ...
    for (let i = 0; i < tm; i++) {
      ...
      i < 2
        ? (salidap = 0)
        : (salidap = parseFloat(controladores[i - 1] * b - a * salidap));
      tiempos.push(i);
      ...
      if (controlActionLabel == namesControlAction[i]) {
        ...
        const relatedEdges = () => {
          ...
          while (idActual != targetSystemId && relations.length > 0) {
            relations = graph
              .getModel()
              .getIncomingEdges(graph.getModel().getCell(actualEdge));
            for (let i = 0; i < relations.length; i++) {
              ...
              if (source.getAttribute("type") == "branchpoint") {
                ...
                relations = graph
                  .getModel()
                  .getOutgoingEdges(graph.getModel().getCell(salida));
                for (let i = 0; i < relations.length; i++) {
