let adaptation_state_main = function adaptation_state_main(graph) {
	...
	data["m_relation_styles"] = adaptation_state_relation_styles();
	return data;
	...
	function adaptation_state_constraints(graph) {
		...
		graph.multiplicities.push(new mxMultiplicity(
			true, "transition", null, null, 0, 1, ["initialState", "state"],
			"Only one connection allowed",
			"The transition only can be associated to states"));
		graph.multiplicities.push(new mxMultiplicity(
			false, "transition", null, null, 0, 1, ["initialState", "state"],
			"Only one connection allowed",
			"The transition only can be associated to states"));
	}
	...
	function adaptation_state_relation_styles() {
		let relations = [];
		relations[0] = {
			"source": ["state", "initialState", "transition"],
			"rel_source_target": "and",
			"target": ["state", "initialState", "transition"],
