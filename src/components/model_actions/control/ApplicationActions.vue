<template>
  <div>
    <a @click="test_micro()" class="dropdown-item">{{
      $t("control_data_acquisition")
    }}</a>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    test_micro() {
      axios.post(
        localStorage["domain_implementation_main_path"] + "Control/saveBD",
        0
      );
      ...
      const createModal = () => {
        ...
        footer.push(
          modalButton("start", function() {
            setInterval(getData, 500);
            document.getElementById("start").disabled = true;
          })
        );
        footer.push(
          modalButton("reset", function() {
            reset();
          })
        );
        footer.push(
          modalButton("export", function() {
            exportCsv();
          })
        );
        footer.push(
          modalButton("send", function() {
            enviar();
          })
        );
      ...
      const exportCsv = () => {
        ...
        let itemSet = sets.indexOf(
          parseInt(document.getElementById("setpoint").value)
        );
        let datacsv;
        let setcsv;
        let multiples = [0];
        ...
        try {
          const removeOffset = () => {
            const average =
              previousData.reduce((a, b) => a + b) / output.length;
